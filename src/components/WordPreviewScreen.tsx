...
export const WordPreviewScreen: React.FC = () => {
  const { state, selectedTopic, generateNewWord, setPhase, skipCurrentWord } = useGame();
    ...
    <div className="flex-1 flex flex-col justify-between px-4 py-4 max-w-md mx-auto w-full text-center safe-bottom">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-1 text-xs font-bold text-amber-400 bg-amber-500/10 px-2.5 py-1 rounded-full border border-amber-500/15">
        ...
        <div className="mb-2">
          ...
          <p className="text-xs text-slate-400 mt-1 max-w-xs mx-auto font-ui">
      ...
      >
        <div className="relative rounded-3xl glass-panel p-6 border border-violet-500/20 shadow-xl overflow-hidden">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/[0.05] border border-white/8 text-slate-300 text-xs font-semibold mb-4 font-ui">
            <span>{selectedTopic?.icon}</span>
            <span>{selectedTopic?.name}</span>
          </div>

          <div className="text-2xl sm:text-3xl font-display font-bold text-white font-mono tracking-wider break-words my-3 py-2">
            {currentWord}
          </div>
